...
class GUI:
    ...
    def updatecharacter(self,objPlayer,playertype,cur_index):
        ...
        self.screen.blit(playChar,(init_posx,init_posy))
...
def heal(t_unit):
    ...
    t_unit.healthpoint += heal_points
    
    # check if unit has exceeded its healthpoint  (100), if so reset to 100 
    if t_unit.healthpoint > 100:
        t_unit.healthpoint = 100
...
def attack(t_attacker,t_target):
    # calculate damage points. use the initialized atk and dep
    dmgpts = calculateDamagePoints(t_attacker.atk,t_attacker.dep)
...
# write game status on text file
def writeToLogfile(t_text):
    print("logging...")
    now = datetime.datetime.now()
    t_filename = dirname + "/logs/" + now.strftime("%Y%m%d") + ".log"
    with open(t_filename, 'w', newline='') as file:
        writer = csv.writer(file, delimiter=',')
        writer.writerows(t_text)
